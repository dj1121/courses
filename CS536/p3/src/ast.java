...
class FnBodyNode extends ASTnode {
    ...
    public void unparse(PrintWriter p, int indent) {
      ...
      p.println("{");
      if(myDeclList != null){
        myDeclList.unparse(p,indent); //dont know if i can do 4
      }
      if(myStmtList != null){
    ...
    public void unparse(PrintWriter p, int indent) {
      try{
        for(int i = 0; i < myStmts.size(); i++){
          ((StmtNode)myStmts.get(i)).unparse(p,indent);
        }
      }catch(NoSuchElementException e){
        System.err.println("unexpected NoSuchElementException in FormalsListNode.print");       
        System.exit(-1);
...
class StructDeclNode extends DeclNode {
    ...
    public void unparse(PrintWriter p, int indent) {
      int upIndent = indent +4;
      addIndent(p,indent);
      p.print("struct ");
      myId.unparse(p,indent);
      p.println("{");
      myDeclList.unparse(p,upIndent);
      p.println("};");
    }
...
class IfElseStmtNode extends StmtNode {
    ...
    public void unparse(PrintWriter p, int indent) {
      ...
      p.println(") {");
      //not sure
      myThenDeclList.unparse(p,upIndent);
      myThenStmtList.unparse(p,upIndent);
      addIndent(p,indent);
      p.println("}");
      addIndent(p,indent);
      p.println("else {");
      myElseDeclList.unparse(p,upIndent);
      myElseStmtList.unparse(p,upIndent);
      addIndent(p,indent);
      p.println("}");
    }
