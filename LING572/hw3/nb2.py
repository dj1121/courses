...
def train(data):
    ...
    for i in range(0, len(separated_data)):
        ...
        for j in range(0, len(counts) - 1):
            ...
            cond_probs[i].append((cond_d + curr_feat_count) / ((len(word_to_index) + np.sum(counts))))
...
def classify(data):
    ...
    for i in range(0, len(data)):
        for j in range(0, len(prior_probs)):
            ...
            # Term 2 Sum N_ik * log(P(w_k | c))
            doc_vector = data[i][0:-1]
            cond_prob_vector = np.log10(cond_probs[j])
            term2 = np.dot(doc_vector, cond_prob_vector)
